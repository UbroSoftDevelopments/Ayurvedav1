...
class StudentController{
    ...
    async studentRegister (req,res){
		...
		if(! firstName || !email ||  !mobile)
			return res.json({status:false,message:'Must provide firstName ,  email , mobile',data:null});
		...
		user.save(err=>{
			if(!err){
				...
				return res.json({status:true,message:'Register successs',data ,password});
			}
			else
				return res.json({status:false,message:`User already found!`,data:err});
		});
	...
	async studentLogin (req,res){
		...
		if(!email || !password)
			return res.json({status:false,message:'Must provide email and password',data:null});
      	
		
		db.Student.findOne({email}).lean().then(async (doc)=>{
			if(!doc)
		  ...
		  //await app.checkPassword(password,doc.password);
		 	if(st){
				...
				return res.json({status:false,message:'Password did not match',data:null});
		}).catch(err=>{
			return res.json({status:false,message:`User already found!`,data:err});
		})
	...
	userSendOtp(req, res) {
    	var { mobile } = req.body;
    		if (!mobile) {
      		res.json({status:false,message:'Must provide mobile number',data:null});
      		return;
    	}

    	db.User.findOne({mobile}).then(async (doc)=>{
			if(!doc)
				return res.json({status:false,message:'User not found!',data:null});
	    ...
	    res.json({status:true,message:'OTP has been sent on your mobile!',data:{otp,token}});
	    return;
		}).catch(err=>{
	...
	updatePassword(req,res){
		...
		if (!mobile || !otp || !password)
      		return res.json({status:false,message:'Must provide mobile number,otp and password',data:null});
